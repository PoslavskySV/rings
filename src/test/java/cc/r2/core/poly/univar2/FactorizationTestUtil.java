...
public final class FactorizationTestUtil {
    ...
    static <T extends IMutablePolynomial<T>> void assertDistinctDegreeFactorization(T poly, FactorDecomposition<T> factorization) {
    ...
    static <E> void assertFactorization(gMutablePolynomial<E> poly, E factor, List<gMutablePolynomial<E>> factorization) {
        assertEquals(poly, factorization.stream().reduce(poly.createConstant(factor), (a, b) -> a.clone().multiply(b)));
    }
