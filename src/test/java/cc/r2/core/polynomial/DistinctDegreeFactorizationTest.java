...
import cc.r2.core.polynomial.FactorizationTestUtil.PolynomialSource;
import cc.r2.core.polynomial.FactorizationTestUtil.RandomSource;
import cc.r2.core.polynomial.FactorizationTestUtil.ShoupSource;
...
import static cc.r2.core.polynomial.DistinctDegreeFactorization.*;
...
public class DistinctDegreeFactorizationTest {
    ...
    @Test
    public void sadasdsa() throws Exception {
        long modulus = BigPrimes.nextPrime(1000000L);

        MutablePolynomial poly = MutablePolynomial.create(1, 2, 3, 1, 3, 1234, 1, 23, 2, 321, 435, 54, 543);
        FactorDecomposition f = DistinctDegreeFactorizationPlain(poly, modulus);

        System.out.println(poly);
        System.out.println(modulus);
        System.out.println(f.toString(false));

    }

    public void test1() throws Exception {
        MutablePolynomial poly = MutablePolynomial.create(0, -1, -1, -1, 0, 1, -1, 1, 1);
        FactorDecomposition f = DistinctDegreeFactorizationPlain(poly, 3);
    ...
    public void test2() throws Exception {
        long modulus = 11;
        MutablePolynomial poly = MutablePolynomial.create(32, 1, 4).multiply(MutablePolynomial.create(3, 3, 1), modulus);
        FactorDecomposition plain = DistinctDegreeFactorizationPlain(poly, modulus);
        FactorDecomposition shoup = DistinctDegreeFactorizationShoup(poly, modulus);
        assertDistinctDegreeFactorization(poly, plain, modulus);
        assertDistinctDegreeFactorization(poly, shoup, modulus);
    ...
    public void test3() throws Exception {
        long modulus = 11;
        MutablePolynomial poly = MutablePolynomial.create(1, 1).multiply(MutablePolynomial.create(4, 9, 1), modulus);
        System.out.println(poly);
        FactorDecomposition plain = DistinctDegreeFactorizationPlain(poly, modulus);
        FactorDecomposition shoup = DistinctDegreeFactorizationShoup(poly, modulus);
    ...
    public void test2a() throws Exception {
        for (int i = 0; i < 1000; i++) {
            ...
            FactorDecomposition f1 = DistinctDegreeFactorizationPlain(bigPoly, bigModulus);
            System.out.println(System.nanoTime() - start);

            start = System.nanoTime();
            FactorDecomposition f2 = DistinctDegreeFactorizationShoup(bigPoly, bigModulus);
            System.out.println(System.nanoTime() - start);
    ...
    enum DDFAlgorithm {
        Plain {
            FactorizationTestUtil.WithTiming<FactorDecomposition> factor(MutablePolynomial poly, long modulus) {
                long start = System.nanoTime();
                FactorDecomposition r = DistinctDegreeFactorizationPlain(poly, modulus);
        ...
        XPowers {
            FactorizationTestUtil.WithTiming<FactorDecomposition> factor(MutablePolynomial poly, long modulus) {
                long start = System.nanoTime();
                FactorDecomposition r = DistinctDegreeFactorizationPrecomputedExponents(poly, modulus);
                long timing = System.nanoTime() - start;
                return new FactorizationTestUtil.WithTiming<>(r, timing);
            }
        },
        Shoup {
            FactorizationTestUtil.WithTiming<FactorDecomposition> factor(MutablePolynomial poly, long modulus) {
                long start = System.nanoTime();
                FactorDecomposition r = DistinctDegreeFactorizationShoup(poly, modulus);
                long timing = System.nanoTime() - start;
                return new FactorizationTestUtil.WithTiming<>(r, timing);
            }
        };
//        ,
//        Shoup2 {
//            WithTiming<FactorDecomposition> factor(MutablePolynomial poly, long modulus) {
//                long start = System.nanoTime();
//                FactorDecomposition r = ShoupDDFFactorization2(poly, modulus);
//                long timing = System.nanoTime() - start;
//                return new WithTiming<>(r, timing);
//            }
//        };

        abstract FactorizationTestUtil.WithTiming<FactorDecomposition> factor(MutablePolynomial poly, long modulus);
    }
        ...
        void run() throws IOException {
            ...
            for (int i = 0; i < nIterations; i++) {
                ...
                for (long modulus : primes) {
                    MutablePolynomial poly = source.take(modulus);
                    degrees.addValue(poly.degree);
                    try {
                        Map<DDFAlgorithm, FactorizationTestUtil.WithTiming<FactorDecomposition>> results = algorithms.stream().collect(Collectors.toMap(al -> al, al -> al.factor(poly, modulus)));
                        DDFAlgorithm baseAlgorithm = algorithms.stream().findFirst().get();
                        FactorDecomposition baseDecomposition = results.get(baseAlgorithm).val;
                        factors.addValue(baseDecomposition.factors.size());
                        results.forEach((alg, res) -> {
                            performance.get(alg).addValue(res.nanoSeconds);

                            FactorDecomposition c = res.val;
                            try {
                                assertDistinctDegreeFactorization(poly, c, modulus);
...
//                                throw err;
    ...
    public void test35a() throws Exception {
        ...
        FactorDecomposition factorization = DistinctDegreeFactorizationPlain(poly, 31);
    ...
    public void test37a() throws Exception {
        ...
        FactorDecomposition factorization = DistinctDegreeFactorizationPlain(poly, 11);
    ...
    public void test38() throws Exception {
        ...
        FactorDecomposition fct = DistinctDegreeFactorizationComplete(poly, 181);
    ...
    static void assertMMATest(int nLines, String file) throws Exception {
        ...
        for (FactorizationTestData fct : FactorizationTestData.allMod(resource)) {
            if (nEntries == 100)
                stats.clear();
            FactorDecomposition expected = fct.factorization;
            long start = System.nanoTime();
//            System.out.println(Arrays.toString(fct.poly.data));
            FactorDecomposition actual = DistinctDegreeFactorizationComplete(fct.poly, fct.modulus);
    ...
    public void testDDFLarge() throws Exception {
        for (int i = 0; i < 100; i++) {
            ...
            long modulus = 5659;
            long start = System.nanoTime();
            FactorDecomposition factorization = DistinctDegreeFactorizationComplete(bigPoly, modulus);
