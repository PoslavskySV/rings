...
public class LargeDDFtest2 {
    ...
    public static void hPowers(MutablePolynomial h, MutablePolynomial polyModulus, InverseModMonomial invMod, long modulus, int nIterations, ArrayList<MutablePolynomial> exponents) {
    ...
    @Test
    public void testHPowers() throws Exception {
        ...
        ArrayList<MutablePolynomial> exps = hPowers(poly, polyModulus, fastDivisionPreConditioning(polyModulus, bigModulus), bigModulus, 10);
        for (int i = 0; i < exps.size(); i++)
            assertEquals(exps.get(i), polyPowMod(poly, LongArithmetics.toInt(i * bigModulus), polyModulus, bigModulus, false));
    }


    /** returns {@code poly^modulus mod polyModulus} **/
    public static MutablePolynomial raiseWithXPowers(MutablePolynomial poly,
...
//        poly = polyMod(poly, polyModulus, invMod, modulus, true);
    ...
    public void testRaiseWithXPowers() throws Exception {
        MutablePolynomial poly = bigPoly;
        ...
        MutablePolynomial raised = poly;
        for (int i = 0; i < 15; i++) {
            MutablePolynomial prev = raised;
            MutablePolynomial next = raiseWithXPowers(raised, poly, invMod, modulus, xPowers);
    ...
    /** returns main(point) mod polyModulus */
    static MutablePolynomial BrentKung(
    ...
    static DDFSteps ShoupDDF(MutablePolynomial poly, long modulus) {
        int n = poly.degree;
        ...
        ArrayList<MutablePolynomial> hPowers = hPowers(xPowerBig, poly, invMod, modulus, tBrentKung);
        for (int i = 0; i < m - 1; ++i)
    ...
    static ArrayList<MutablePolynomial> ShoupDDFFactorization(MutablePolynomial poly, long modulus) {
        return ShoupDDFFactorization(poly, modulus, ShoupDDF(poly, modulus));
    }

    private static ArrayList<MutablePolynomial> ShoupDDFFactorization(MutablePolynomial poly, long modulus, DDFSteps steps) {
    ...
    static final class DDFSteps {
        ...
        public DDFSteps(int b, int l, int m, ArrayList<MutablePolynomial> babySteps, ArrayList<MutablePolynomial> giantSteps, InverseModMonomial invMod) {
            B = b;
            this.l = l;
            this.m = m;
    ...
    public void testDDF1() throws Exception {
        long modulus = 3;
        MutablePolynomial poly = RandomPolynomials.randomMonicPoly(50, modulus, new Well1024a());
        DDFSteps ddfSteps = ShoupDDF(poly, modulus);

        for (int i = 0; i <= ddfSteps.l; i++)
            assertEquals(polyMod(
                    MutablePolynomial.createMonomial(1, (int) LongArithmetics.pow(modulus, i)), poly, modulus, false), ddfSteps.babySteps.get(i));

        for (int i = 0; i <= ddfSteps.m; i++) {
            MutablePolynomial expected = polyPowMod(MutablePolynomial.createMonomial(1, 1), LongArithmetics.pow(modulus, ddfSteps.l * i), poly, modulus, false);
    ...
    public void testDDF1a() throws Exception {
        long modulus = 2;
        MutablePolynomial poly = bigPoly.clone().cut(50);
        poly.modulus(modulus).monic(modulus);
        DDFSteps ddfSteps = ShoupDDF(poly, modulus);

        for (int i = 0; i <= ddfSteps.l; i++) {
            MutablePolynomial expected = polyPowMod(MutablePolynomial.createMonomial(1, 1), LongArithmetics.pow(modulus, i), poly, modulus, false);
...
//        MutablePolynomial poly = RandomPolynomials.randomMonicPoly(12, bigModulus, new Well1024a()).multiply(RandomPolynomials.randomMonicPoly(250,modulus,new Well1024a()));
        MutablePolynomial poly = bigPoly;//.clone().multiply(bigPoly, modulus).square(modulus).add(bigPoly.cut(10), modulus);
//        poly = polyPowMod(poly, 10, modulus, false).add(poly.clone().cut(100), modulus).add(MutablePolynomial.one(), modulus);
//        poly = poly.cut(3000);
        poly.modulus(modulus).monic(modulus);
        ...
        DDFSteps ddfSteps = ShoupDDF(poly, modulus);
        ArrayList<MutablePolynomial> f = ShoupDDFFactorization(poly, modulus, ddfSteps);
        System.out.println(f.size());
//        System.out.println(f);
        MutablePolynomial e = MutablePolynomial.one();
        for (MutablePolynomial factor : f) {
