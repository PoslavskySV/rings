...
object implicits {
  ...
  object univariate {
    ...
    = self match {
      ...
      case _ => throw new RuntimeException("unknown type: " + self.getClass + "  " + self.coefficientDomainToString())
    }
  }
  ...
  (self: E)(implicit ring: PolynomialRing[Poly, E]): RingOperators[E] =
    ring match {
      case r: GaloisField64 => RingOperatorsZp64(self.asInstanceOf[Long], r.domain.factory.domain).asInstanceOf[RingOperators[E]]
      case r: GaloisField[E] => new RingOperators[E](self)(domainAsRing(r.factory.domain))
