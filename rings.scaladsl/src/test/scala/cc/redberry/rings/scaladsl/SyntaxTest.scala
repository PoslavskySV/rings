...
class SyntaxTest {
  ...
  def testGenericFunction: Unit = {
...

    {
      ...
      assertEquals(
        ...
        UnivariateInterpolation.interpolateNewton(ring.cfRingZp64, points, values)
      )
    }
  }
  ...
  def complicatedGaloisField: Unit = {
    ...
    assert(IrreduciblePolynomials.irreducibleQ(gf0.getMinimalPolynomial))

    val gf1 = GF(gf0Ring("(1 + x + x^3) + (1 + x^2) * y + y^2 - 6 * y^3 + y^4 - (1 + x + x^3) * y^6 + y^7"), "y")(gf0Ring.cfRing)
    val gf1Ring = UnivariateRing(gf1, "z")
    assert(IrreduciblePolynomials.irreducibleQ(gf1.getMinimalPolynomial))
