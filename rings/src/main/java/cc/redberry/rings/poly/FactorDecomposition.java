...
/**
 * Factor decomposition of polynomial. Constant factor of decomposition is stored in {@link #constantFactor}, factors
 * returned by {@link #get(int)} are non-constant. This class is mutable.
 *
 * <p><i>Iterable</i> specification provides iterator over non-constant factors only; to iterate over all factors
...
public final class FactorDecomposition<Poly extends IPolynomial<Poly>>
    ...
    FactorDecomposition<PolyT> map(Function<Poly, PolyT> mapper) {
        return FactorDecomposition.of(mapper.apply(constantFactor),
    ...
    /** Array of factors (constant factor is the last in the array) */
    public Poly[] toArray() {
        if (constantFactor.isOne())
    ...
    private static <P extends IPolynomial<P>> String toStringFactorization(List<P> factors, TIntArrayList exponents,
        ...
        return toStringFactorization(factors, exponents, factor, infoAsExponents, vars == null ? ToStringSupport.plain() : ToStringSupport.withVariables(vars));
    }
